...
def thermosteric_hgt_anom(row, testbit=False):
...
def halosteric_hgt_anom(row, testbit=False):
    ...
    if len(pressure) < 2 or not all(pressure[i] < pressure[i + 1] for i in range(len(pressure) - 1)) or len(sal_abs) == 0:
        return [None]
