...
for fn in glob.glob(os.path.join(source_dir, '*.nc')):
    ...
    if position_qc or juld_qc or startup or apex or pressure_sort or no_realtime or require_delayed or region or emptyset:
    ...
    # post level-filter profile exclusion
    ## empty profile
    if len(pres) == 0 or len(temp) == 0 or len(psal) == 0:
        emptyset = True
    if emptyset:
        rejects.loc[len(rejects)-1]['emptyset'] = emptyset
        continue
