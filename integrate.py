...
def parse_pairs(s):
    """
    Parse a string containing ordered pairs into a list of tuples.
    Expected input format: "1,2 3,4 5,6"
    """
    pairs = s.split()  # split on whitespace to get individual pairs
    result = []
    for pair in pairs:
        parts = pair.split(',')
        if len(parts) != 2:
            raise argparse.ArgumentTypeError(
                f"Each pair must contain exactly two values separated by a comma. Got: '{pair}'"
            )
        try:
            first = float(parts[0])
            ...
            raise argparse.ArgumentTypeError(
                f"Could not convert values to float in pair: '{pair}'"
            )
...
parser.add_argument('--regions', type=parse_pairs, required=True,help='A list of integration regions denoted as min0,max0 min1,max1..., e.g. "--integration \'1,2 3,4 5,6\'"')
