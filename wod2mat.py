...
def filterQC(t,s,p, t_qc,s_qc,p_qc, acceptable):
	...
	data = list(zip(t,s,p,t_qc,s_qc,p_qc))
	goodTPS = list(filter(lambda level: level[3] in acceptable and level[4] in acceptable and level[5] in acceptable, data))
...
for file in files:
	...
	while True:
		...
		temp,psal,pres = filterQC(p.t(), p.s(), p.p(), p.t_level_qc(originator=False), p.s_level_qc(originator=False), p.var_level_qc(pindex), [0])
...
scipy.io.savemat('wod.mat', df.to_dict("list"))