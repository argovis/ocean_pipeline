...
for a in unique_argovis:
    ...
    if len(profile) == 1:
        ...
        pres_scrub = [x for x in xar[pressure_var].to_dict()['data'][0] if not numpy.isnan(x)]
        temp_scrub = [x for x in xar[temp_var].to_dict()['data'][0] if not numpy.isnan(x)]
        psal_scrub = [x for x in xar[psal_var].to_dict()['data'][0] if not numpy.isnan(x)]
        ...
        pres_qc = [int(qc.decode()) if isinstance(qc, bytes) else 0 if numpy.isnan(qc) else int(qc) for qc in pres_qc]  # else 0 will allow missing QC to pass the QC filters, which matches PSC's behavior
        temp_qc = [int(qc.decode()) if isinstance(qc, bytes) else 0 if numpy.isnan(qc) else int(qc) for qc in temp_qc]
        psal_qc = [int(qc.decode()) if isinstance(qc, bytes) else 0 if numpy.isnan(qc) else int(qc) for qc in psal_qc]
        ...
        if 20 in xar['PRES_ADJUSTED_ERROR'].to_dict()['data'][0]:
            ...
            negpres.append(id)
        elif any(qc>2 for qc in pres_qc) or any(qc>2 for qc in temp_qc) or any(qc>2 for qc in psal_qc) or int(xar['JULD_QC'].to_dict()['data'][0].decode())>2 or int(xar['POSITION_QC'].to_dict()['data'][0].decode())>2:
            badoldqc.append(id)
